...
public class ThingControllerTest {
    ...
    public void testNewThing() throws Exception {
        String name = "Thing";
        Thing b = new Thing(name);
        ResultActions result = doPost(this.mockMvc, b, "/things", JsonMvcTestHelper::expectOK);
        result
    ...
    public void testUpdateThing() throws Exception {
        String newName = "Thing.x";
        long id = 1L;
        Thing p = new Thing(newName);
        
        doPut(this.mockMvc, p, "/things/" + id, JsonMvcTestHelper::expectOK)
            .andExpect(jsonPath("$._id").value(hasToString(valueOf(id))))
